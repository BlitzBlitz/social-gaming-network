...
export class ChatAdapter2Service extends ChatAdapter {
  ...
  listFriends(): Observable<ParticipantResponse[]> {
    // return this.dataService.getFriends().pipe(map(friends =>{
    //
    //
    //
    ...
    return of(ChatAdapter2Service.mockedParticipants.map(user => {
  ...
  sendMessage(message: Message): void {
    setTimeout(() => {
      ...
      if (isNaN(message.toId)) {
        let group = ChatAdapter2Service.mockedParticipants.find(x => x.id == message.toId) as Group;
        ...
        let user = ChatAdapter2Service.mockedParticipants.find(x => x.id == replyMessage.fromId);
  ...
  //for group games later on

  // groupCreated(group: Group): void {
  //   ChatAdapter2Service.mockedParticipants.push(group);
  //
  //   ChatAdapter2Service.mockedParticipants = ChatAdapter2Service.mockedParticipants.sort((first, second) =>
